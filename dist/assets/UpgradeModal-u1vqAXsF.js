...
import { D as Dialog, l as DialogContent, m as DialogHeader, n as DialogTitle, G as DialogDescription, B as Badge, O as DialogFooter, j as Button } from "./ui-components-CbrOUI4e.js";
import { aA as Crown, T as TrendingUp, Z as Zap } from "./icons-CFSiufIk.js";
function useSubscriptionLimits() {
  const { user } = useAuthStore();
  const { data: subscription, isLoading: subscriptionLoading } = useQuery({
    queryKey: ["subscription", user == null ? void 0 : user.id],
    queryFn: async () => {
  ...
  const { data: usage, isLoading: usageLoading } = useQuery({
    queryKey: ["usage", user == null ? void 0 : user.id],
    queryFn: async () => {
      if (!(user == null ? void 0 : user.id)) return { listings: 0, links: 0, testimonials: 0 };
  ...
  const canAdd = (feature) => {
    ...
    const limit = subscription["max_".concat(feature)];
    const current = usage[feature];
    if (limit === -1) return true;
    return current < limit;
  };
  const hasFeature = (feature) => {
    return (subscription == null ? void 0 : subscription[feature]) || false;
  };
  ...
  const getUsage = (feature) => {
    return (usage == null ? void 0 : usage[feature]) || 0;
  };
