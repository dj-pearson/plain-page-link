;
(function () {
  System.register(['./supabase-legacy-CQONYrP8.js', './react-vendor-legacy-BZejsv6W.js', './charts-legacy-D2SqRQVB.js', './utils-legacy-B2316hnE.js'], function (exports, module) {
    'use strict';

    var supabase, autoPopulateBlockConfig, requireReact, React, getDefaultExportFromCjs, ue;
    return {
      setters: [module => {
        supabase = module.s;
        autoPopulateBlockConfig = module.a;
      }, module => {
      ...
      execute: function () {
        ...
        const useAuthStore = exports("u", create()(persist((set, get) => ({
          user: null,
          session: null,
          profile: null,
          role: null,
          ...
          initialize: async () => {
            ...
            try {
              ...
              if (session?.user) {
                const [profileResult, rolesResult] = await Promise.all([supabase.from("profiles").select("*").eq("id", session.user.id).single(), supabase.from("user_roles").select("role").eq("user_id", session.user.id).order("role", {
                  ascending: true
                })]);
          ...
          signIn: async (email, password) => {
            ...
            try {
              ...
              const {
                data: userRoles
              } = await supabase.from("user_roles").select("role").eq("user_id", data.user.id).order("role", {
                ascending: true
              });
        ...
        class PageBuilderEngine {
          ...
          async addBlock(page, blockType, position, userId) {
            ...
            if (userId) {
              blockConfig = await autoPopulateBlockConfig(template.defaultConfig, userId);
            }
            const newBlock = {
              id: this.generateId(),
              type: blockType,
          ...
          updateBlock(page, blockId, config) {
            const updatedBlocks = page.blocks.map(block => block.id === blockId ? {
              ...block,
              config: {
                ...block.config,
                ...config
          ...
          validatePage(page) {
            ...
            if (!/^[a-z0-9-]+$/i.test(page.slug)) {
              errors.push("Page slug must contain only letters, numbers, and hyphens");
            }
        ...
        const createNewPage = exports("c", pageBuilder.createNewPage.bind(pageBuilder));
        ...
        const getBlockTemplates = exports("g", pageBuilder.getBlockTemplates.bind(pageBuilder));
        pageBuilder.validatePage.bind(pageBuilder);
        const usePageBuilderStore = exports("a", create((set, get) => ({
          // Initial state
          page: null,
          ...
          loadPage: async pageId => {
            try {
              const {
                data,
                error
              } = await supabase.from("custom_pages").select("*").eq("id", pageId).single();
              if (error) throw error;
          ...
          loadUserPages: async () => {
            try {
              ...
              const {
                data,
                error
              } = await supabase.from("custom_pages").select("*").eq("user_id", user.id).order("updated_at", {
                ascending: false
              });
              if (error) throw error;
              return (data || []).map(d => ({
          ...
          savePage: async () => {
            ...
            try {
              ...
              const {
                data: existing
              } = await supabase.from("custom_pages").select("id").eq("id", page.id).single();
              if (existing) {
                const {
                  error
                } = await supabase.from("custom_pages").update(pageData).eq("id", page.id);
                if (error) throw error;
              } else {
                const {
                  error
          ...
          setAsActivePage: async pageId => {
            try {
              ...
              const {
                ...
                is_active: true
              }).eq("id", pageId).eq("user_id", user.id);
              if (error) throw error;
