...
import { c as clsx } from "./utils-DRaK7sdV.js";
import { r as reactExports, R as React } from "./react-vendor-MTOt5FFF.js";
...
class Path {
  ...
  moveTo(x2, y2) {
    this._append(_a || (_a = __template(["M", ",", ""])), this._x0 = this._x1 = +x2, this._y0 = this._y1 = +y2);
  }
  closePath() {
    if (this._x1 !== null) {
      this._x1 = this._x0, this._y1 = this._y0;
      this._append(_b || (_b = __template(["Z"])));
    }
  }
  lineTo(x2, y2) {
    this._append(_c || (_c = __template(["L", ",", ""])), this._x1 = +x2, this._y1 = +y2);
  }
  quadraticCurveTo(x1, y1, x2, y2) {
    this._append(_d || (_d = __template(["Q", ",", ",", ",", ""])), +x1, +y1, this._x1 = +x2, this._y1 = +y2);
  }
  bezierCurveTo(x1, y1, x2, y2, x3, y3) {
    this._append(_e || (_e = __template(["C", ",", ",", ",", ",", ",", ""])), +x1, +y1, +x2, +y2, this._x1 = +x3, this._y1 = +y3);
  }
  arcTo(x1, y1, x2, y2, r) {
    ...
    if (this._x1 === null) {
      this._append(_f || (_f = __template(["M", ",", ""])), this._x1 = x1, this._y1 = y1);
    } else if (!(l01_2 > epsilon)) ;
    else if (!(Math.abs(y01 * x21 - y21 * x01) > epsilon) || !r) {
      this._append(_g || (_g = __template(["L", ",", ""])), this._x1 = x1, this._y1 = y1);
    } else {
      let x20 = x2 - x0, y20 = y2 - y0, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
      if (Math.abs(t01 - 1) > epsilon) {
        this._append(_h || (_h = __template(["L", ",", ""])), x1 + t01 * x01, y1 + t01 * y01);
      }
      this._append(_i || (_i = __template(["A", ",", ",0,0,", ",", ",", ""])), r, r, +(y01 * x20 > x01 * y20), this._x1 = x1 + t21 * x21, this._y1 = y1 + t21 * y21);
    }
  }
  arc(x2, y2, r, a0, a1, ccw) {
    ...
    if (this._x1 === null) {
      this._append(_j || (_j = __template(["M", ",", ""])), x0, y0);
    } else if (Math.abs(this._x1 - x0) > epsilon || Math.abs(this._y1 - y0) > epsilon) {
      this._append(_k || (_k = __template(["L", ",", ""])), x0, y0);
    }
    if (!r) return;
    if (da < 0) da = da % tau + tau;
    if (da > tauEpsilon) {
      this._append(_l || (_l = __template(["A", ",", ",0,1,", ",", ",", "A", ",", ",0,1,", ",", ",", ""])), r, r, cw, x2 - dx, y2 - dy, r, r, cw, this._x1 = x0, this._y1 = y0);
    } else if (da > epsilon) {
      this._append(_m || (_m = __template(["A", ",", ",0,", ",", ",", ",", ""])), r, r, +(da >= pi), cw, this._x1 = x2 + r * Math.cos(a1), this._y1 = y2 + r * Math.sin(a1));
    }
  }
  rect(x2, y2, w, h) {
    this._append(_n || (_n = __template(["M", ",", "h", "v", "h", "Z"])), this._x0 = this._x1 = +x2, this._y0 = this._y1 = +y2, w = +w, +h, -w);
  }
  toString() {
    return this._;
  }
...
var _defineProperty$D;
var hasRequired_defineProperty;
function require_defineProperty() {
  if (hasRequired_defineProperty) return _defineProperty$D;
  ...
  _defineProperty$D = defineProperty;
...
function getTickValuesFixedDomainFn(_ref7, tickCount) {
  ...
  var values = [].concat(_toConsumableArray$9(Arithmetic.rangeStep(new Decimal(cormin), new Decimal(cormax).sub(new Decimal(0.99).mul(step)), step)), [cormax]);
  return min2 > max2 ? reverse(values) : values;
}
...
var Animate = /* @__PURE__ */ (function(_PureComponent) {
  _inherits$b(Animate2, _PureComponent);
  var _super = _createSuper(Animate2);
  function Animate2(props, context) {
    var _this;
    _classCallCheck$d(this, Animate2);
    ...
    _this.handleStyleChange = _this.handleStyleChange.bind(_assertThisInitialized$b(_this));
    _this.changeStyle = _this.changeStyle.bind(_assertThisInitialized$b(_this));
