...
import { D as Dialog, l as DialogContent, m as DialogHeader, n as DialogTitle, G as DialogDescription, L as Label, I as Input, S as Select, a as SelectTrigger, b as SelectValue, d as SelectContent, e as SelectItem, T as Textarea, O as DialogFooter, j as Button } from "./ui-components-CbrOUI4e.js";
import { a6 as Upload, X, u as Share2, F as Facebook, ak as Twitter, f as Linkedin, b as Check, ae as Copy, I as Instagram, L as LoaderCircle, J as CircleCheckBig, aw as Plus, S as Search, ax as Filter, ay as SquarePen, az as Trash2 } from "./icons-CFSiufIk.js";
import { u as useToast } from "./index-CAwD2FR9.js";
import { u as useListings } from "./useListings-Dh7P6Y-u.js";
...
function useListingImageUpload() {
  ...
  const validateFile = (file) => {
    if (!ACCEPTED_FILE_TYPES.includes(file.type)) {
      return "Invalid file type: ".concat(file.name, ". Please upload JPG, PNG, or WEBP images");
    }
    if (file.size > MAX_FILE_SIZE) {
      return "File too large: ".concat(file.name, ". Maximum size is 5MB");
    }
    return null;
  };
  const uploadListingImages = async (files, listingId) => {
    ...
    if (files.length > MAX_IMAGES) {
      toast({
        ...
        description: "You can upload a maximum of ".concat(MAX_IMAGES, " images per listing"),
        variant: "destructive"
      });
      return [];
    }
    ...
    try {
      for (let i = 0; i < files.length; i++) {
        ...
        const fileName = "".concat(user.id, "/").concat(listingId || timestamp, "/").concat(randomId, ".").concat(fileExt);
        ...
        if (uploadError) {
          ...
          throw new Error("Failed to upload ".concat(file.name, ": ").concat(uploadError.message));
        }
      ...
      if (uploadedUrls.length > 0) {
        toast({
          title: "Success",
          description: "Successfully uploaded ".concat(uploadedUrls.length, " image").concat(uploadedUrls.length > 1 ? "s" : "")
        });
      }
      return uploadedUrls;
    } catch (error) {
  ...
  const deleteListingImages = async (imageUrls) => {
    if (!(user == null ? void 0 : user.id) || imageUrls.length === 0) {
      return false;
    }
    try {
...
function SocialShareDialog({
  ...
  const generatePostText = () => {
    ...
    parts.push("");
    parts.push("📍 ".concat(listing.address, ", ").concat(listing.city));
    parts.push("💰 ".concat(formatPrice(listing.price)));
    if (listing.beds || listing.baths) {
      const details = [];
      if (listing.beds) details.push("".concat(listing.beds, " beds"));
      if (listing.baths) details.push("".concat(listing.baths, " baths"));
      parts.push("🛏️ ".concat(details.join(" | ")));
    }
    if (listing.sqft) {
      parts.push("📐 ".concat(listing.sqft.toLocaleString(), " sq ft"));
    }
    parts.push("");
    parts.push("Don't miss out on this ".concat(listing.property_type || "amazing property", "!"));
    parts.push("");
    ...
    if (listing.city) {
      parts.push("#".concat(listing.city.replace(/\s+/g, ""), "RealEstate"));
    }
    return parts.join("\n");
  };
  ...
  const shareToFacebook = () => {
    ...
    window.open("https://www.facebook.com/sharer/sharer.php?u=".concat(url, "&quote=").concat(text), "_blank");
  };
  const shareToTwitter = () => {
    const text = encodeURIComponent(postText.substring(0, 280));
    window.open("https://twitter.com/intent/tweet?text=".concat(text), "_blank");
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: "New Listing: ".concat(listing.address),
          text: postText
        });
      } catch (err) {
...
function QuickStatusUpdate({
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      ...
      const statusLabel = ((_a = statusOptions.find((s) => s.value === newStatus)) == null ? void 0 : _a.label) || newStatus;
      toast({
        title: "Status Updated",
        description: "Listing marked as ".concat(statusLabel)
      });
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "relative inline-block", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Select, { value: status, onValueChange: handleStatusChange, disabled: isUpdating, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      SelectTrigger,
      {
        className: "w-[140px] h-8 text-xs ".concat((currentOption == null ? void 0 : currentOption.color) || "", " ").concat(isUpdating ? "opacity-50" : ""),
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 sm:space-y-6", children: [
    ...
    newlyCreatedListing && /* @__PURE__ */ jsxRuntimeExports.jsx(
      SocialShareDialog,
      {
        ...
        agentName: profile == null ? void 0 : profile.full_name
      }
    ),
