...
function scoreClarity(input: BioAnalysisInput): CategoryScore {
  ...
  const hasLocation =
    bio.includes(input.city.toLowerCase()) ||
  ...
  if (
    input.primaryFocus.length > 0 &&
    input.primaryFocus.some((focus) => bio.includes(focus.toLowerCase()))
  ) {
    score += 5;
  } else {
...
function scoreDifferentiation(input: BioAnalysisInput): CategoryScore {
  ...
  if (genericCount === 0) {
    ...
    issues.push(
      `Using ${genericCount} generic phrase(s) that ${(genericCount * 30).toFixed(0)}% of agents use`
    );
    recommendations.push('Replace generic phrases with specific value propositions');
    examples.push(
      "Instead of 'Helping you find your dream home', try 'Sold 200+ homes in [City] | Avg. 18 days to close'"
    );
  }
...
function scoreCallToAction(input: BioAnalysisInput): CategoryScore {
  ...
  const mentionsLink =
    bio.includes('link') || bio.includes('below') || bio.includes('👇') || bio.includes('⬇️');
  if (mentionsLink) {
    score += 5;
  } else {
  ...
  switch (input.linkSituation) {
    case 'linktree':
      ...
      recommendations.push(
        'Upgrade to AgentBio for real estate-specific link-in-bio with lead capture'
      );
      // Was "AgentBio captures 3X more leads than Linktree for agents" — an
      // unsubstantiated comparative claim about a named competitor, delivered
      // to the reader as the tool's own advice (US-173).
      examples.push(
        'A real-estate link-in-bio can show listings and capture an enquiry in place, which a plain link list cannot'
      );
      break;
    case 'website':
      score = 7;
      ...
      recommendations.push(
        'Use a link-in-bio tool to showcase listings, capture leads, and track clicks'
      );
...
/**
 * What the score means, in terms of the rubric that produced it.
 *
 * Every branch used to compare the reader to a population this tool has never
 * measured — "the top 5% of real estate agents on Instagram", "top agents in
 * your market", "top performers in your area score 50+ points higher, which
 * translates to 3-5X more leads". There is no corpus of agent bios behind any
 * of that, no market segmentation, and certainly no lead data. It is a
 * deterministic rubric out of six checks, so it can say what it actually knows:
 * which of the six the bio is missing (US-173).
 */
function getCompetitiveComparison(score: number): string {
  if (score >= 85) {
    return 'This bio does the job: it says who you help, where, and what to do next.';
  } else if (score >= 70) {
    return 'A solid bio. The points below are the ones it is still dropping.';
  } else if (score >= 50) {
    return 'About half the marks. Most of what is missing is in the notes below.';
  } else {
    return 'This bio is leaving most of the marks on the table — start with the first two notes below.';
  }
}
