...
export class PushNotificationManager {
  ...
  private async registerServiceWorker(): Promise<void> {
    ...
    try {
      // Register FCM under its own dedicated scope so it does not evict the
      // app's caching/offline service worker (public/sw.js), which registers
      // at the root scope '/'. A scope can hold only one registration.
  ...
  async getToken(): Promise<string | null> {
    ...
    try {
      ...
      // Pass the explicitly-scoped registration so Firebase uses it instead
      // of auto-registering firebase-messaging-sw.js at the root scope.
