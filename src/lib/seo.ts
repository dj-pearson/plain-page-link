...
export const generateRealEstateAgentSchema = (page: PageConfig): Record<string, any> => {
  ...
  if (listingsBlock) {
    schema.numberOfAvailableAccommodations = (listingsBlock as any).config.maxItems || 0;
  }
 ...
 */
export const generateStructuredData = (page: PageConfig): Record<string, any> => {
  // Combine multiple schemas using @graph
  const schemas = [generatePersonSchema(page), generateRealEstateAgentSchema(page)];
...
export const generateMetaTags = (page: PageConfig): string[] => {
  ...
  tags.push(`<meta name="keywords" content="${page.seo.keywords.join(', ')}" />`);

  // Canonical URL
  tags.push(`<link rel="canonical" href="${generateCanonicalUrl(page.slug)}" />`);

  // Open Graph tags
  tags.push(`<meta property="og:title" content="${preview.title}" />`);
  tags.push(`<meta property="og:description" content="${preview.description}" />`);
  ...
  tags.push(`<meta name="twitter:description" content="${preview.description}" />`);
...
export const generatePricingSchema = (
  ...
  return {
    ...
    name: 'AgentBio',
    description:
      'Purpose-built link-in-bio platform for real estate agents with property galleries, lead capture, and appointment booking.',
    brand: {
      '@type': 'Brand',
      name: 'AgentBio',
    },
