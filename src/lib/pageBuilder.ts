...
import { PageBlock, PageConfig, BlockType, BlockConfig, BlockTemplate } from '@/types/pageBuilder';
...
class PageBuilderEngine {
   ...
   */
  updateBlock(page: PageConfig, blockId: string, config: Partial<BlockConfig>): PageConfig {
    const updatedBlocks: PageBlock[] = page.blocks.map((block) =>
      block.id === blockId
        ? {
            ...block,
            // Both sides are the BlockConfig union, so spreading one
            // into the other yields every cross-product of the twelve
            // member types — 200-odd shapes, none of which is
            // BlockConfig. The patch is only meaningful when it
            // belongs to this block's own type, which is the caller's
            // responsibility; asserted once here rather than at each
            // call site.
            config: { ...block.config, ...config } as BlockConfig,
          }
        : block
    );
   ...
   * Reorder blocks
   */
  reorderBlocks(page: PageConfig, sourceIndex: number, destinationIndex: number): PageConfig {
    const updatedBlocks = [...page.blocks];
...
export const toggleBlockVisibility = pageBuilder.toggleBlockVisibility.bind(pageBuilder);
export const duplicateBlock = pageBuilder.duplicateBlock.bind(pageBuilder);
export const getBlockTemplates = pageBuilder.getBlockTemplates.bind(pageBuilder);
