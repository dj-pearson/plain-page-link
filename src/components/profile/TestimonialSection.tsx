...
export function TestimonialSection({ testimonials }: TestimonialSectionProps) {
  ...
  const averageRating =
    safeTestimonials.length > 0
      ? safeTestimonials.reduce((sum, t) => sum + t.rating, 0) / safeTestimonials.length
      : 0;
  ...
  // Nothing to render with no testimonials. Guard placed after all hooks so
  // they are always called in the same order (rules of hooks).
    ...
    <section className="w-full max-w-6xl mx-auto px-4 py-12">
      ...
      <div className="md:hidden">
        <div ref={swipeRef} className="relative select-none">
          <TestimonialCard testimonial={sortedTestimonials[currentIndex]} variant="default" />
