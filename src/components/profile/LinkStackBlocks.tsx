...
/**
 * Parses a `custom_css` declaration string into a React style object.
 *
 * This used to be `style={{ cssText: link.custom_css }}`. `cssText` is a
 * property of the DOM's CSSStyleDeclaration, not a React style key — React
 * only applies keys it recognises, so the custom CSS was dropped in full and
 * no link ever rendered with it.
 *
 * Parsing into an object rather than injecting a <style> tag is also the safer
 * of the two: React writes each value through the style property setter, so a
 * malformed or hostile declaration cannot escape into a selector or an @import.
 */
function parseInlineCss(css: string): CSSProperties {
  ...
  for (const declaration of css.split(';')) {
    ...
    // React accepts camelCase properties, and custom properties verbatim.
...
export default function LinkStackBlocks({ links, onLinkClick }: LinkStackBlocksProps) {
  ...
  const renderBlock = (link: LinkStackLink) => {
    switch (link.type) {
      case 'spacer':
        return <div key={link.id} className="w-full" style={{ height: '32px' }} />;

      case 'heading':
        return (
          <div key={link.id} className="w-full text-center py-2">
            <h3 className="text-2xl font-bold text-gray-900">{link.title}</h3>
          </div>
        );

      case 'text':
        return (
          <div key={link.id} className="w-full text-center py-2 px-4">
      ...
      case 'link':
        return (
          <a
            ...
            style={link.custom_css ? parseInlineCss(link.custom_css) : undefined}
          >
            <div className="flex items-center justify-center gap-3">
              {link.custom_icon && <i className={link.custom_icon}></i>}
      ...
      case 'predefined':
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center gap-3">
              {platform?.icon && <i className={platform.icon}></i>}
              <span className="font-semibold">{platform?.title || link.title}</span>
            </div>
          </a>
        );
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="font-semibold">{link.title || 'Call Me'}</span>
            </div>
          </a>
        );
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="font-semibold">{link.title || 'Email Me'}</span>
            </div>
          </a>
        );
        ...
        return (
          ...
          >
            <div className="flex items-center justify-center gap-3">
              ...
              <span className="font-semibold">{link.title || 'Save Contact'}</span>
            </div>
          </a>
        );
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-3 py-6">
      <div className="text-center mb-4">
        <h2 className="text-xl font-semibold text-gray-900">More Links</h2>
      </div>
