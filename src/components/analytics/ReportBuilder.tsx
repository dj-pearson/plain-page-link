...
export interface ReportConfig {
  reportType: 'leads' | 'listings' | 'conversions' | 'sources' | 'performance';
...
export function ReportBuilder({ onGenerateReport, className }: ReportBuilderProps) {
  const [reportType, setReportType] = useState<ReportConfig['reportType']>('leads');
  const [dateRange, setDateRange] = useState<ReportConfig['dateRange']>('30d');
    ...
    try {
      ...
      if (exportFormat === 'csv') {
        ...
        if (data && data.length > 0) {
          ...
          // Awaited: generatePDFReport loads jsPDF on demand
          // (US-162), so the success toast must follow it and a
          // failure must reach the catch below.
    ...
    switch (type) {
      ...
      case 'conversions':
        const totalConversions = data.reduce(
          (sum: number, d: any) => sum + (d.conversions || 0),
          0
        );
        return [
      ...
      case 'sources':
        return [
          ...
          {
            label: 'Total Leads',
            value: data.reduce((sum: number, d: any) => sum + (d.count || 0), 0),
          },
        ];
      default:
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select value={exportFormat} onValueChange={(v: any) => setExportFormat(v)}>
            ...
            <SelectContent>
              {formats.map((fmt) => (
                <SelectItem key={fmt.value} value={fmt.value} disabled={fmt.disabled}>
                  {fmt.label}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <div className="space-y-1 text-sm text-gray-600">
          ...
          <p>
            ...
            <span className="font-medium">
              {format(getDateRangeForReport().start, 'MMM d, yyyy')} -{' '}
              {format(getDateRangeForReport().end, 'MMM d, yyyy')}
            </span>
          </p>
          <p>
            Format: <span className="font-medium uppercase">{exportFormat}</span>
          </p>
        </div>
      </div>
    </div>
