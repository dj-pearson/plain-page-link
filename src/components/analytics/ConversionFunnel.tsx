  ...
  return (
    <div className={cn('bg-white p-6 rounded-lg border', className)}>
      ...
      <div className="space-y-2">
        {stages.map((stage, index) => (
          <div key={stage.name}>
            ...
            />
            {index !== stages.length - 1 && stage.dropoff !== undefined && stage.dropoff > 0 && (
              <DropoffIndicator
                count={stage.dropoff}
      ...
      <div className="mt-6 pt-6 border-t grid grid-cols-3 gap-4">
        ...
        />
        <StatItem label="Total Dropoff" value={calculateTotalDropoff(stages).toLocaleString()} />
      </div>
    </div>
  );
}
...
function FunnelStageCard({ stage, maxCount, isFirst }: FunnelStageCardProps) {
      ...
      >
        <div className="flex items-center justify-between text-white">
          <div>
            ...
            <p className="text-sm opacity-90">{stage.count.toLocaleString()} leads</p>
          </div>
          <div className="text-right">
            <p className="text-2xl font-bold">{stage.percentage.toFixed(1)}%</p>
            {!isFirst && <p className="text-xs opacity-90">of total</p>}
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-center py-2">
      <div className="flex items-center gap-2 text-sm text-gray-600">
        ...
        <span>
          <span className="font-medium text-red-600">{count.toLocaleString()}</span> dropped off (
...
function calculateDropoffPercentage(current: FunnelStage, next: FunnelStage): number {
