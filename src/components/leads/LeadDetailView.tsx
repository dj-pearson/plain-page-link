...
export function LeadDetailView({
  ...
  return (
    <div className={cn('fixed inset-0 bg-black/50 z-50 overflow-y-auto', className)}>
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl">
          ...
          <div className="p-6 border-b">
            ...
            <div className="mt-4 flex gap-2">
              <Button className="gap-2" onClick={() => window.open(`mailto:${lead.email}`)}>
                <Mail className="w-4 h-4" />
                Email
              </Button>
              {lead.phone && (
              ...
              {onScheduleCall && (
                <Button variant="outline" className="gap-2" onClick={onScheduleCall}>
              ...
              <Select
                value={lead.status}
                onValueChange={(v) => onUpdateStatus(v as Lead['status'])}
              >
            ...
            {activeTab === 'timeline' && (
              <div className="space-y-4">
                {lead.timeline && lead.timeline.length > 0 ? (
                  lead.timeline.map((event) => <TimelineItem key={event.id} event={event} />)
                ) : (
