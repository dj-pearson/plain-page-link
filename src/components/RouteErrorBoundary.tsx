...
/**
 * Detects a failed lazy-chunk import (typically after a deploy invalidated the
 * old chunk hashes). These cannot be recovered by re-rendering — React.lazy
 * caches the rejected promise — so the only fix is a full page reload.
 */
        ...
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 border border-gray-100">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">
                {chunkError ? 'Update Required' : `${section} Error`}
              </h2>
          ...
          <div className="mb-6 p-4 bg-gray-50 rounded-lg">
            <p className="text-sm text-gray-700">
              {chunkError
                ? 'The application has been updated. Please reload the page to load the latest version.'
                : `An error occurred while rendering the ${section.toLowerCase()} section. Other parts of the application should still work normally.`}
            </p>
          </div>

          {import.meta.env.DEV && this.state.error && (
            <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
              <p className="text-xs font-mono text-red-800 mb-1">Dev Error Details:</p>
