...
export function StaleContentAlert({
  ...
  return (
    <div className={cn('bg-orange-50 border-2 border-orange-200 rounded-lg', className)}>
      {/* Header */}
      <div
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-orange-900">Stale Listings Need Attention</h3>
            <p className="text-sm text-orange-700">
              {criticalCount > 0 && (
                <span className="font-medium">{criticalCount} critical (14+ days)</span>
              )}
              {criticalCount > 0 && warningCount > 0 && ' · '}
              {warningCount > 0 && <span>{warningCount} warning (7+ days)</span>}
            </p>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="p-4 bg-orange-100/50 border-t border-orange-200">
          <p className="text-xs text-orange-800 mb-2 font-medium">💡 Quick Tips:</p>
          <ul className="text-xs text-orange-700 space-y-1 ml-4 list-disc">
            <li>Update listing descriptions with recent improvements or price changes</li>
            <li>Add new photos to refresh your listing's appeal</li>
            <li>Verify pricing is still competitive in current market</li>
...
function StaleListingRow({ listing, onDismiss, onRefresh }: StaleListingRowProps) {
    ...
    <div className="p-4 flex items-center justify-between hover:bg-orange-100/30 transition-colors">
      <div className="flex-1">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-600">
          ...
          <span className="font-medium text-orange-700">{listing.daysSinceUpdate} days old</span>
        </div>
      </div>
      <div className="flex gap-2 ml-4">
        <Button size="sm" variant="outline" onClick={onRefresh} className="gap-1 bg-white">
