...
import { CheckSquare, Square, Edit, Trash2, Save, X } from 'lucide-react';
...
export function BulkEditMode({
  ...
  const [priceAdjustType, setPriceAdjustType] = useState<'increase' | 'decrease' | 'set'>(
    'increase'
  );
      ...
      {isEditing && selectedIds.length > 0 && (
        <div className="p-6 bg-white rounded-lg border-2 border-primary shadow-lg">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-semibold">Bulk Edit {selectedIds.length} Listings</h3>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                    ...
                    {priceValue && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-primary">
                          ${calculatePricePreview(listing).toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                {selectedListings.length > 5 && (
                  <p className="text-xs text-gray-500 text-center">
                    ... and {selectedListings.length - 5} more
                  </p>
                )}
              </div>
            </div>
