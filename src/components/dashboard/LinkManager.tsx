...
export default function LinkManager({ userId }: LinkManagerProps) {
  ...
  const handleToggleVisibility = (id: number) => {
    setLinks(
      links.map((link) =>
        link.id === id ? { ...link, up_link: link.up_link === 'yes' ? 'no' : 'yes' } : link
      )
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-2">Custom Links</h2>
          <p className="text-gray-600">
            Manage your link-in-bio blocks with drag-and-drop ordering
          </p>
        </div>
        <Button onClick={openAddModal} className="bg-blue-600 text-white hover:bg-blue-700">
      ...
      {links.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-2 font-semibold">No links yet</p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-500 truncate">
                    {link.type === 'spacer' ? 'Spacer Block' : link.link || 'No URL'}
                  </div>
                </div>

                {/* Stats */}
                <div className="text-sm text-gray-600">{link.click_number} clicks</div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    title={link.up_link === 'yes' ? 'Hide' : 'Show'}
                  >
                    {link.up_link === 'yes' ? (
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingLink ? 'Edit Block' : 'Add New Block'}</DialogTitle>
            <DialogDescription>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="predefined">📱 Social Media (Predefined)</SelectItem>
                  <SelectItem value="heading">📰 Heading</SelectItem>
                  <SelectItem value="text">📝 Text Block</SelectItem>
                  <SelectItem value="spacer">➖ Spacer</SelectItem>
                  <SelectItem value="telephone">📞 Phone Number</SelectItem>
