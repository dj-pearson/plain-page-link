...
import { Home, Clock, CheckCircle, XCircle, RefreshCw } from 'lucide-react';
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">
            Click listings to select, then use buttons or keyboard shortcuts
          </p>
        </div>
        <Button
          variant="outline"
...
function StatsCard({ label, count, active, onClick, color, alert }: StatsCardProps) {
  const colors = {
  ...
  return (
    ...
    >
      ...
      <div className={cn('text-sm font-medium', color && colors[color])}>{label}</div>
    </button>
  );
}
...
function ListingQuickCard({
  ...
  return (
    ...
    >
      ...
      <div className="mt-6">
        <h3 className="font-semibold text-sm truncate mb-1">{listing.title}</h3>
        <p className="text-lg font-bold text-primary mb-2">${listing.price.toLocaleString()}</p>
        ...
        {/* Quick Stats */}
        {(listing.views !== undefined || listing.leads !== undefined) && (
          <div className="flex gap-3 text-xs text-gray-600 mb-3">
            {listing.views !== undefined && <span>👁️ {listing.views} views</span>}
            {listing.leads !== undefined && <span>👥 {listing.leads} leads</span>}
          </div>
        )}
