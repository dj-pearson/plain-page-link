...
export function CameraUpload({
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (file.size > maxSizeMB * 1024 * 1024 * 10) {
          toast.error(`${file.name} is too large (max ${maxSizeMB * 10}MB before compression)`);
          continue;
        }
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      {loading && <div className="text-center text-sm text-gray-600">Processing images...</div>}

      <p className="text-xs text-gray-500">
        {previews.length}/{maxFiles} images • Max {maxSizeMB}MB per image
      </p>
    </div>
  );
}
