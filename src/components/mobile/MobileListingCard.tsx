...
export function MobileListingCard({
  ...
  // The same rule the desktop dashboard applies. This used to be `> 7` with
  // no status test, so a listing untouched for exactly a week was stale on
  // one surface and not the other, and a SOLD listing was nagged about here
  // and nowhere else — on the device an agent is most likely holding
  // (US-194).
  const isStale = () => isListingStale(listing);
        ...
        <div className="flex gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => onStatusChange?.(listing.id, 'pending')}>
                    Mark Pending
                  </DropdownMenuItem>
            ...
            <div className="flex items-center gap-3 text-xs text-gray-600 mt-1">
              ...
              <span
                className={cn('px-2 py-0.5 rounded-full font-medium', statusColors[listing.status])}
              >
            ...
            <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
              ...
              {isStale() && (
                <span className="text-orange-700 font-medium" title={staleExplanation(listing)}>
                  (Stale)
