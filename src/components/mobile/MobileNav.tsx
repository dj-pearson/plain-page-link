...
import { PRIMARY_NAV, SETTINGS_TOOLS } from '@/config/dashboard-nav';
...
export function MobileNav() {
  ...
  // From src/config/dashboard-nav.ts, the same source the sidebar renders
  // from. These two lists used to be maintained separately and had drifted:
  // /dashboard/subscription, /team and /api-keys were in the sidebar and
  // nowhere on mobile, so an agent on a phone could not reach their own
  // billing (US-120).
  //
  // The bar holds the first four; the sheet holds everything else, so every
  // destination either nav offers is reachable here.
  const navItems: NavItem[] = PRIMARY_NAV.slice(0, 4).map((item) => ({
    label: item.label,
    icon: item.icon,
    href: item.href,
  }));

  const secondaryNavItems = [...PRIMARY_NAV.slice(4), ...SETTINGS_TOOLS].map((item) => ({
    label: item.label,
    icon: item.icon,
    href: item.href,
  }));
