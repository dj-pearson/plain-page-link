...
export function VoiceInput({
  ...
  useEffect(() => {
    ...
    recognition.onstart = () => {
      logger.debug('[VoiceInput] Speech recognition started');
      setIsListening(true);
    };

    recognition.onend = () => {
      logger.debug('[VoiceInput] Speech recognition ended');
      setIsListening(false);
    };

    recognition.onerror = (event: any) => {
      logger.error('[VoiceInput] Speech recognition error', event.error);
      setIsListening(false);
    ...
    recognition.onresult = (event: any) => {
      ...
      if (finalTranscript) {
        ...
        if (newValue.length <= maxLength) {
          ...
          toast.warning(`Maximum length of ${maxLength} characters reached`);
          recognition.stop();
        }
      }
    };
    ...
    <div className={cn('relative', className)}>
      ...
      {isSupported ? (
        <Button
          ...
          disabled={disabled}
          className={cn('absolute bottom-2 right-2', isListening && 'animate-pulse')}
