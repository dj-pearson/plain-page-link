...
export function ContactBlock({ config, isEditing = false, userId }: ContactBlockProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true);
      toast.success(config.successMessage || 'Message sent successfully!');
  ...
  return (
    <div className="max-w-md mx-auto">
      {/* Title */}
      {config.title && <h3 className="text-2xl font-bold text-center mb-6">{config.title}</h3>}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {field.type === 'checkbox' && (
              <div className="flex items-center gap-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => handleChange(field.id, Boolean(checked))}
                />
        ...
        {/* Submit Button */}
        <Button type="submit" className="w-full gap-2" disabled={isEditing || isSubmitting}>
