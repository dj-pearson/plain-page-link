...
import { themePresets, availableFonts, colorPalettes, isValidHexColor } from '@/lib/themes';
...
export function ThemeCustomizer({ theme, onThemeChange }: ThemeCustomizerProps) {
  ...
  const handleBorderRadiusChange = (radius: 'none' | 'small' | 'medium' | 'large' | 'full') => {
    onThemeChange({
      ...theme,
      borderRadius: radius,
    });
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['compact', 'normal', 'spacious'] as const).map((spacing) => (
                <Button
                  key={spacing}
                  variant={theme.spacing === spacing ? 'default' : 'outline'}
                  size="sm"
