...
export default function LeadCapture() {
  ...
  const pageSchema = {
    ...
    description:
      'Capture qualified buyer and seller leads with pre-built forms. Buyer inquiries, seller leads, and home valuation requests—all optimized for real estate.',
    mainEntity: {
  ...
  return (
    <>
      {/* No citationDate: it was new Date() — "written today", asserted on
                every render (US-167). Nothing here knows when the page last
                changed, so it says nothing. */}
      <SEOHead
      ...
      <main id="main-content" className="min-h-screen bg-background" tabIndex={-1}>
        ...
        <section className="py-4 bg-background/50 border-b border-glass-border">
          <div className="container mx-auto px-4">
            <Breadcrumb
              /* The "Features" rung used to point at
                               /features/property-listings — a sibling feature
                               page, not a parent — and both URLs were relative,
                               which Google rejects outright. There is no
                               /features route in App.tsx (see the note in
                               prerender-routes.ts; US-149 took it out of the
                               sitemap as a soft 404), so the honest trail is two
                               rungs rather than an invented three. US-171 is the
                               question of whether a real hub is worth building. */
              items={[
                { name: 'Home', url: getCanonicalUrl('/') },
                { name: 'Lead Capture', url: getCanonicalUrl('/features/lead-capture') },
              ]}
            />
          </div>
        </section>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              <p className="text-base md:text-lg glass-body leading-relaxed">
                <strong>
                  AgentBio's lead capture forms help real estate agents qualify buyer and seller
                  leads
                </strong>{' '}
                with pre-built forms for buyer inquiries, seller leads, and home valuation requests.
                Unlike generic contact forms, each form includes pre-qualification questions like
                budget, timeline, and motivation—giving you the information needed to prioritize hot
                leads and follow up effectively with instant email notifications.
              </p>
            </div>
          </div>
        </section>
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="p-8 rounded-xl bg-glass-background backdrop-blur-md border border-glass-border text-center">
                ...
                <p className="glass-body text-muted-foreground">
                  Pre-approved buyer looking in 30 days? Call immediately. Browsing with no
                  timeline? Nurture campaign.
                </p>
              </div>
            </div>
          </div>
