...
export default function Login() {
  ...
  // Only navigate once the user is authenticated AND any required MFA
  // challenge has been satisfied. When MFA is pending we render the
  // verification step below instead of granting access.
  const mfaPending = !!user && requiresMFA && !mfaVerified;
  ...
  }, [user, mfaPending, navigate, redirectTo]);

  const handleCancelMFA = async () => {
    try {
      await signOut?.();
    } catch (err) {
      logger.error('Failed to sign out during MFA cancel', err as Error);
    }
  };
    ...
    try {
      ...
      if (isNetworkError) {
        toast({
          ...
          description:
            'Your browser cache may be interfering. Try refreshing the page or clearing your browser cache.',
          variant: 'destructive',
        });
        setTimeout(() => {
          cleanupAndReload();
  ...
  // When the password step succeeds but the account has MFA enabled, show the
  // second-factor challenge instead of the credential form.
  if (mfaPending) {
    return (
      <main
        id="main-content"
      ...
      >
        <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          <MFAVerification onSuccess={() => setMFAVerified?.(true)} onCancel={handleCancelMFA} />
        </div>
      </main>
    );
  }
  ...
  return (
    <main id="main-content" tabIndex={-1} className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900">
        ...
        <div
          className="absolute bottom-1/4 right-0 w-96 h-96 bg-indigo-500/15 rounded-full blur-3xl animate-pulse"
