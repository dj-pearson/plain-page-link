...
export default function Links() {
  ...
  const activeCount = useMemo(() => visibleLinks.filter((l) => l.is_active).length, [visibleLinks]);
    ...
    try {
      ...
      toast({
        ...
        description: userFacingError(error, {
          subject: 'link',
          fallback: 'Failed to add link. Please try again.',
        }),
        variant: 'destructive',
      });
    }
  ...
  const handleUpdateLink = async (id: string, data: Partial<Link>) => {
    try {
      ...
      toast({
        ...
        description: userFacingError(error, {
          subject: 'link',
          fallback: 'Failed to update link. Please try again.',
        }),
        variant: 'destructive',
      });
    }
  ...
  const handleMoveLink = useCallback(
    async (link: Link, direction: 'up' | 'down') => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {subscription && getLimit('links') !== Infinity && (
        <LimitBanner feature="links" current={getUsage('links')} limit={getLimit('links')} />
      )}
          ...
          {isLoading ? (
            ...
            <div className="p-8 sm:p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4 max-w-sm mx-auto">
                Add links to your social profiles, Zillow page, scheduling tool, and more so
                visitors can find you everywhere.
              </p>
              <button
                onClick={handleAddClick}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="sm:hidden mt-1 text-xs text-muted-foreground">
                      <span className="font-semibold text-foreground">{link.click_count}</span>{' '}
                      clicks
                    </div>
                  </div>
                  ...
                  <div className="text-right hidden sm:block flex-shrink-0 min-w-[60px]">
                    <div className="text-lg font-semibold text-foreground">{link.click_count}</div>
                  ...
                  <div
                    ...
                    title={link.is_active ? 'Visible on profile' : 'Hidden from profile'}
                  >
                    <Switch
      ...
      {!isError && visibleLinks.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900 rounded-lg p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-blue-800 dark:text-blue-200 leading-relaxed">
            <strong>Tips:</strong> Use the arrow buttons to reorder links. Toggle the switch to
            show/hide links on your profile. The order here matches what visitors see.
          </p>
        </div>
      )}

      {/* Add Link Modal */}
      <AddLinkModal open={isAddModalOpen} onOpenChange={setIsAddModalOpen} onSave={handleAddLink} />
