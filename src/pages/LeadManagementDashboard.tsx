...
import { ResponseTemplates, DEFAULT_TEMPLATES } from '@/components/leads/ResponseTemplates';
import { HotLeadAlert, HotLeadIndicator } from '@/components/leads/HotLeadAlert';
...
const mockLeads = [
  ...
  },
  {
    ...
    lastContactedAt: new Date(Date.now() - 1000 * 60 * 60 * 3).toISOString(),
  },
  {
    id: '5',
    name: 'Lisa Anderson',
...
export default function LeadManagementDashboard() {
  ...
  const handleMarkAsRead = async (leadId: string) => {
    setLeads((prev) =>
      prev.map((lead) => (lead.id === leadId ? { ...lead, status: 'contacted' as const } : lead))
    );
    toast.success('Lead marked as contacted');
  };

  const handleUpdateStatus = async (status: (typeof mockLeads)[0]['status']) => {
  ...
  const handleCreateTemplate = (template: Omit<(typeof DEFAULT_TEMPLATES)[0], 'id'>) => {
    const newTemplate = {
      ...template,
      id: `custom-${Date.now()}`,
    };
  ...
  const handleUpdateTemplate = (id: string, updates: Partial<(typeof DEFAULT_TEMPLATES)[0]>) => {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage your leads efficiently with smart scoring and quick responses
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <StatCard label="Total Leads" value={stats.total} icon={<Mail className="w-5 h-5" />} />
        <StatCard
          label="New"
          value={stats.new}
  ...
  return (
    <div className="p-4 bg-white rounded-lg border hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between mb-2">
        <div className={`p-2 rounded ${color ? colorClasses[color] : 'bg-gray-100 text-gray-600'}`}>
          {icon}
        </div>
