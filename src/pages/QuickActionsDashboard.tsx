...
import { AnalyticsWidget, defaultRealEstateStats } from '@/components/dashboard/AnalyticsWidget';
    ...
    try {
      for (const id of selectedIds) {
        ...
        if (updates.priceAdjustment) {
          ...
          if (type === 'increase') {
            newPrice =
              unit === 'percent' ? listing.price * (1 + value / 100) : listing.price + value;
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Quick Actions Dashboard</h1>
          <p className="text-gray-600 mt-1">
            Manage your listings efficiently with keyboard shortcuts and bulk actions
          </p>
        </div>
        {staleListings.length > 0 && <StaleContentBadge count={staleListings.length} />}
      </div>
      ...
      {staleListings.length > 0 && (
        <StaleContentAlert staleListings={staleListings} onRefresh={handleRefreshStale} />
      )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
              {listings.map((listing) => (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 capitalize">{listing.status}</p>
                </div>
              ))}
            </div>
          </TabsContent>
